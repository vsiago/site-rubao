...
const ImageCropper = ({ imageSrc }) => {
  ...
  const overlayImageSrc = "/tema-estou-com-rubao20.png"; // Substitua pelo caminho da sua imagem de overlay
  ...
  const applyBlurBackground = (imageSrc) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        ctx.drawImage(canvas, 0, 0, 380, 380);
  ...
  const handleSave = async () => {
    ...
    blurredBackgroundImg.onload = () => {
      ...
      combinedCanvas.width = 380;
      combinedCanvas.height = 380;
      const ctx = combinedCanvas.getContext("2d");

      ctx.drawImage(blurredBackgroundImg, 0, 0, 380, 380);
      ctx.drawImage(croppedCanvas, 0, 0, 380, 380);

      const overlayImage = new Image();
      overlayImage.src = overlayImageSrc;
      overlayImage.onload = () => {
        ctx.drawImage(overlayImage, 0, 0, 380, 380);
